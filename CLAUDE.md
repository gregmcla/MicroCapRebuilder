...
- **`max_positions` cap on AI-driven proposals is opt-in** (`enforce_max_positions: true` in config). Without the flag, Claude's BUY list passes through unfiltered (advisory cap only — historical behavior of the 35 portfolios that existed before 2026-05-06). With the flag, `_validate_allocation` truncates BUYs to fit `held_after_sells + new_buys <= max_positions`. New portfolios created via `portfolio_registry.create_portfolio()` get the flag set automatically. Existing portfolios are NOT migrated — opt them in by hand if/when desired.
