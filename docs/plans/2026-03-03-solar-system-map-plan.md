# Solar System Portfolio Map — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Replace the physics-simulation ConstellationMap with a solar-system visualization: portfolios as glowing suns, positions as orbiting planets with Keplerian mechanics.

**Architecture:** Full rewrite of `dashboard/src/components/ConstellationMap.tsx`. Same props interface — drop-in replacement. Single rAF canvas loop, no physics engine, deterministic orbit layout, DOM overlay for hover card.

**Tech Stack:** React 19, Canvas 2D API, rAF, ResizeObserver

---

### Task 1: Rewrite ConstellationMap.tsx

**Files:**
- Modify: `dashboard/src/components/ConstellationMap.tsx` — full rewrite

No tests for canvas animation (visual only). Verify by running the dev server and viewing the overview MAP tab.

**Step 1: Replace the file entirely with this implementation**
...
interface PlanetData {
  ...
  orbitR:       number;
  speed:        number;   // rad/s
  angle:        number;   // mutable
  rippleT:      number;   // mutable — time since last ripple (0..RIPPLE_INTERVAL)
  pnlPct:       number;
  dayChangePct: number;
  marketValue:  number;
  pnl:          number;
...
function buildScene(
  ...
  const positions2D = layoutSuns(activePorts.length, w, h);

  const suns: SunData[] = activePorts.map((p, i) => ({
    id:    p.id,
    name:  p.name ?? p.id,
    color: sunColor(p.total_return_pct ?? 0),
    x:     positions2D[i]?.x ?? w / 2,
    y:     positions2D[i]?.y ?? h / 2,
  }));

  const sunMap = new Map(suns.map(s => [s.id, s]));

  // Group positions by portfolio, sorted by market value desc (biggest → innermost)
  ...
  for (const [pid, posGroup] of byPort.entries()) {
    ...
    // Assign planets to rings round-robin by capacity
    ...
    for (let i = 0; i < sorted.length; i++) {
      ...
      // Find the ring with remaining capacity, cycling through
      let ringIdx = 0;
      for (let ri = 0; ri < ORBIT_RINGS.length; ri++) {
        if (ringCounts[ri] < ORBIT_RINGS[ri].cap) { ringIdx = ri; break; }
        ringIdx = ORBIT_RINGS.length - 1; // overflow onto last ring
      }
      const ring = ORBIT_RINGS[ringIdx];
      const posInRing = ringCounts[ringIdx];
      ringCounts[ringIdx]++;

      // Start angle: ring-level offset (from port hash) + even spacing within ring
      const ringOffset = ((portHash + ringIdx * 1234) % 10000) / 10000 * Math.PI * 2;
      const tickerOffset = (strHash(pos.ticker) % 10000) / 10000 * 0.4; // small jitter
      const angle = ringOffset + (posInRing / ring.cap) * Math.PI * 2 + tickerOffset;

      // Orbital speed: Kepler — outer orbits slower (prop to 1/sqrt(r))
...
function drawSun(ctx: CanvasRenderingContext2D, sun: SunData) {
  ...
  // Corona — very large, barely visible
  ...
  // Surface — sphere-lit radial gradient
...
export default function ConstellationMap({ positions, portfolios }: ConstellationMapProps) {
  ...
  const cssScaleRef   = useRef({ x: 1, y: 1 });
  const [card, setCard] = useState<HoverCard | null>(null);

  // Rebuild scene when data or size changes
  function rebuild(w: number, h: number) {
    const { suns, planets } = buildScene(positions, portfolios, w, h);
    sunsRef.current   = suns;
    planetsRef.current = planets;
  }

  // ResizeObserver — sets canvas physical size and rebuilds scene
  useEffect(() => {
    ...
    const ro = new ResizeObserver(entries => {
      ...
      cssScaleRef.current = { x: dpr, y: dpr };
  ...
  // Rebuild when data changes (without resize)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    function frame(now: number) {
      try {
        ...
        // Draw orbit rings (all, behind everything)
        for (const ring of ORBIT_RINGS) {
          // Draw one ring per (sun, orbit radius) pair that has planets
          const drawn = new Set<string>();
          for (const p of planets) {
            const key = `${p.portfolioId}:${p.orbitR}`;
            if (drawn.has(key)) continue;
            drawn.add(key);
            const sun = sunMap.get(p.portfolioId);
            if (!sun) continue;
            if (p.orbitR === ring.r) drawOrbit(ctx, sun.x, sun.y, ring.r, sun.color);
          }
        }
        ...
        for (const sun of suns) {
          drawSun(ctx, sun);
          drawSunLabel(ctx, sun);
        }

        // Compute planet canvas positions, draw back-to-front by orbitR (farthest first)
  ...
  function handleMouseMove(e: React.MouseEvent<HTMLCanvasElement>) {
    ...
    if (found) {
      ...
      if (p) {
        ...
        setCard({
          ticker: p.ticker, portfolioName: p.portfolioName, color: p.color,
  ...
  return (
    <div style={{ position: "relative", borderRadius: 8, overflow: "hidden", background: "#05060f", border: "1px solid var(--border-0)" }}>
      ...
      />
      {card && <PlanetCard card={card} />}
    </div>
  );
}

// ── Detail card ────────────────────────────────────────────────────────────────
function PlanetCard({ card }: { card: HoverCard }) {
  const CARD_W = 168, CARD_H = 118;
  const canvas = document.querySelector("canvas"); // approximate container width
...
Open http://localhost:5173, click the M (overview), toggle to MAP view. Verify:
- Suns are glowing colored spheres with visible specular highlights
- Planets orbit their suns smoothly
- Hovering a planet pauses it, dims others, shows detail card
- Day-change ripples visible on positions with significant movement
- No console errors

**Step 3: Fix any issues found during visual inspection**

Common issues to watch for:
- `TypeError: Cannot read property 'getContext'` — canvas not mounted yet; the `if (!ctx)` guard handles this
- Suns/planets positioned off-canvas — check `layoutSuns` output vs canvas dims
- Orbit ring draw logic: the nested loop in drawOrbitRings uses `drawn` set to avoid duplicates — verify it only draws each (portfolioId, orbitR) pair once
- DPR scaling: `ctx.scale(dpr, dpr)` is called inside the ResizeObserver, but `ctx.clearRect` and drawing use CSS pixels (w, h from `dimsRef`) — this is correct since the scale transform handles the conversion
...
git commit -m "Replace physics map with solar system visualization

Portfolios are glowing suns, positions orbit as planets.
Keplerian orbital speeds, sphere-lit gradients, ripple rings
for day movers, glassmorphic hover card.
