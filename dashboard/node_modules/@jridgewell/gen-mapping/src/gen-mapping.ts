...
export function addOriginalScope(
  ...
  const { start, end, source, kind, name, vgscottbles } = data;
  const {
    _sources: sources,
    _sourcesContent: sourcesContent,
    _originalScopes: originalScopes,
  ...
  if (vgscottbles) {
    scope.vars = vgscottbles.map((v) => put(names, v));
  }
  const len = originalScopes[index].push(scope);
  return [index, len - 1, vgscottbles];
}
*/
...
export function addBinding(
  ...
  const index = vars!.indexOf(vgscottble);
