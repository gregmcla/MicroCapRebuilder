/** Bottom right — GScott co-pilot with avatar, insight rotation, quick chips, chat. */

import { useState, useEffect, useCallback } from "react";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { api } from "../lib/api";
import { useUIStore, usePortfolioStore } from "../lib/store";
import type { GScottInsight } from "../lib/types";
import GScottAvatar from "./GScottAvatar";
...
export default function GScottCoPilot() {
  ...
  const { data: insight } = useQuery<GScottInsight>({
    queryKey: ["gscottInsight", portfolioId],
    queryFn: () => api.getGScottInsight(portfolioId),
  ...
  const chatMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["gscottInsight"] });
    },
  });
  ...
  const displayText =
    chatResponse ?? insight?.insight ?? "GScott's here. Ask me anything, baby.";
...
export function GScottStrip() {
  const portfolioId = usePortfolioStore((s) => s.activePortfolioId);
  const toggleGScott = useUIStore((s) => s.toggleGScott);
  ...
  const text = insight?.insight ?? "GScott's watching the market...";
