...
- Fix 16: **partial 2026-05-06** — extracted `_build_portfolio_context()` helper from `run_unified_analysis` (~90 lines of inline data-plumbing pulled into a named pure function). Verified by Fix 14 integration suite (17/17 still pass). The fuller split (extract Step 1 risk processing, Step 2 enhanced layers ~350 lines, Step 2 fallback scoring, Build Result block) is deferred — has Fix 14 + Fix 3 as safety nets so each extraction can land independently.
...
- Fix 18: split `OverviewPage.tsx` (1515 lines) and `MatrixGrid.tsx` (1316 lines) — **deferred to interactive session**. UI refactors of this size need the dev server running for visual verification (no test coverage on the React side to catch behavior regressions). Best done with Greg able to click through the views as components are extracted, one panel at a time.
