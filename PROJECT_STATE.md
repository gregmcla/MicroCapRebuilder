...
- Fix 19: cache infrastructure refactor — 14 caches in system, only 2 hash their inputs. Config edits silently serve stale results until TTL expires (greg hit this live 2026-05-06: edited cluster-ignition `industries`, screener_cache + refinement_cache served stale results, had to hand-delete files). Spec: (a) every cache key = hash of inputs; (b) tiered TTLs by data volatility (universe → invalidate at market open + on config-hash change; bars → 1h market / 12h overnight; fundamentals → 30d; AI outputs → ephemeral); (c) cache observability — log every hit with age + key; (d) global cache-bust endpoint per portfolio; (e) **content validation at read time** — verify cached data matches the request (e.g. MultiIndex ticker label = requested ticker), reject + refetch on mismatch. Audit table in conversation 2026-05-06.
